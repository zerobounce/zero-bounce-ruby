...
{\*\expandedcolortbl;;}
\paperw11900\paperh16840\margl1440\margr1440\vieww20560\viewh22780\viewkind0
\pard\tx566\tx1133\tx1700\tx2267\tx2834\tx3401\tx3968\tx4535\tx5102\tx5669\tx6236\tx6803\pardirnatural\partightenfactor0

\f0\fs24 \cf0 # Zerobounce\
Cliente Ruby para la API de Zerobounce.net. Desarrollado basado en [esta versi\'f3n](https://github.com/afrase/zerobounce) por Aaron Frase.\
\
## Uso\
Agrega esta l\'ednea a tu Gemfile de la aplicaci\'f3n:\
```ruby\
gem 'zerobounce-sdk'\
```\
\
...
```\
\
O inst\'e1lalo directamente con:\
```bash\
...
```\
\
Configura una clave de API v\'e1lida de ZeroBounce:\
```ruby\
...
```ruby\
Zerobounce.credits\
=> 79   # devuelve -1 si la clave es inv\'e1lida\
```\
\
Actividad\
```ruby\
...
```\
\
### Validaci\'f3n de correo electr\'f3nico\
\
Validar un solo correo electr\'f3nico\
```ruby\
Zerobounce.validate('valid@example.com') \
# Zerobounce.validate('valid@example.com', '192.168.0.1') # direcci\'f3n IP opcional\
=>\
...
# Zerobounce.validate_batch(emails, ip_addresses) # par\'e1metro ip_addresses opcional\
 \
...
```\
\
#### Validaci\'f3n de archivo en lote\
\
Enviar archivo\
```ruby\
...
\{"success"=>true,\
 ...
 # se requerir\'e1 el file_id para los pr\'f3ximos pasos\
 # par\'e1metros opcionales: \
    ...
    gender_column: 4,\
    has_header_row: true,\
    return_url: nil         # URL de devoluci\'f3n de resultados\
...
## Desarrollo\
\
Despu\'e9s de revisar el repositorio, ejecuta las pruebas:\
```bash\
...
```\
\
### Par\'e1metros de prueba\
Las pruebas utilizan los siguientes par\'e1metros de entorno:\
TEST \{unit|live\} influye en si se ejecutan las pruebas simuladas de unidad o se utiliza el servidor en vivo (se pueden utilizar cr\'e9ditos si eliges hacer esto)\
ZEROBOUNCE_API_KEY \{<zerobounce-api-key-value>\} esta clave se utiliza para realizar solicitudes al servidor en vivo; tambi\'e9n se utiliza en las pruebas simuladas como un ejemplo de clave v\'e1lida (cualquier valor funcionar\'e1 para las pruebas simuladas)\
INCORRECT_API_KEY \{cualquier cadena sin espacios en blanco que no sea una clave v\'e1lida\} se utiliza para pruebas en las que las solicitudes deben fallar debido al valor de la clave de la API.\
\
Para configurarlos:\
```bash\
...
\
Las pruebas simuladas se generaron utilizando webmock y vcr. Esto significa que se realizaron solicitudes reales y se grabaron en los archivos .yml bajo spec/cassettes con una clave API v\'e1lida en ese momento utilizada con fines de prueba. Sin embargo, esta clave se ha invalidado en el \'ednterin, pero se proporciona en el archivo .env.sample para que las pruebas simuladas funcionen. Si no desea utilizar esta clave para simulaciones, puede reemplazarla con cualquier valor en los archivos .yml bajo spec/cassettes o eliminarlos todos y volver a ejecutar las pruebas para que vcr los grabe con una nueva clave.}